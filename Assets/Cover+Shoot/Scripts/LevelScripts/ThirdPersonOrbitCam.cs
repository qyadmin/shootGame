...
public class ThirdPersonOrbitCam : MonoBehaviour 
{
    ...
    void Update()
	{
		...
		angleV = Mathf.LerpAngle(angleV, recoilAngle, 10f*Time.deltaTime);
        ...
        Vector3 pos = new Vector3(player.position.x,player.position.y+ differential, player.position.z);

        cam.position = pos + camYRotation * smoothPivotOffset + aimRotation * smoothCamOffset;
   ...
   [SerializeField]
    private float differential = 0;
    [SerializeField]
    float actionspeed = 1;
    public void SquatDown()
    {
        IEnumerator squatdown_aciton = moveCamera(action.SquatDown);
        StartCoroutine(squatdown_aciton);
    }
    public void StandUp()
    {
        IEnumerator standup_aciton = moveCamera(action.StandUp);
        StartCoroutine(standup_aciton);
    }

    IEnumerator moveCamera(action value)
    {
        switch (value)
        {
            case action.StandUp:
                ...
                {
                    differential += actionspeed * Time.deltaTime;
                    yield return null;
                }
                break;
            case action.SquatDown:
                while (differential >= -0.8)
                {
                    differential -= actionspeed * Time.deltaTime;
                    yield return null;
                }
